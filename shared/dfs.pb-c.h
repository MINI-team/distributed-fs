/* Generated by the protocol buffer compiler.  DO NOT EDIT! */
/* Generated from: dfs.proto */

#ifndef PROTOBUF_C_dfs_2eproto__INCLUDED
#define PROTOBUF_C_dfs_2eproto__INCLUDED

#include <protobuf-c/protobuf-c.h>

PROTOBUF_C__BEGIN_DECLS

#if PROTOBUF_C_VERSION_NUMBER < 1003000
# error This file was generated by a newer version of protoc-c which is incompatible with your libprotobuf-c headers. Please update your headers.
#elif 1003003 < PROTOBUF_C_MIN_COMPILER_VERSION
# error This file was generated by an older version of protoc-c which is incompatible with your libprotobuf-c headers. Please regenerate this file with a newer version of protoc-c.
#endif


typedef struct _FileRequestRead FileRequestRead;
typedef struct _FileRequestWrite FileRequestWrite;
typedef struct _Replica Replica;
typedef struct _Chunk Chunk;
typedef struct _ChunkList ChunkList;
typedef struct _ChunkRequest ChunkRequest;
typedef struct _WriteChunk WriteChunk;
typedef struct _FileResponse FileResponse;
typedef struct _CommitChunk CommitChunk;
typedef struct _NewReplica NewReplica;
typedef struct _ChunkCommitReport ChunkCommitReport;


/* --- enums --- */


/* --- messages --- */

struct  _FileRequestRead
{
  ProtobufCMessage base;
  char *path;
};
#define FILE_REQUEST_READ__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&file_request_read__descriptor) \
    , (char *)protobuf_c_empty_string }


struct  _FileRequestWrite
{
  ProtobufCMessage base;
  char *path;
  int64_t size;
};
#define FILE_REQUEST_WRITE__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&file_request_write__descriptor) \
    , (char *)protobuf_c_empty_string, 0 }


struct  _Replica
{
  ProtobufCMessage base;
  char *name;
  char *ip;
  int32_t port;
  protobuf_c_boolean is_primary;
};
#define REPLICA__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&replica__descriptor) \
    , (char *)protobuf_c_empty_string, (char *)protobuf_c_empty_string, 0, 0 }


struct  _Chunk
{
  ProtobufCMessage base;
  int32_t chunk_id;
  char *path;
  size_t n_replicas;
  Replica **replicas;
};
#define CHUNK__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&chunk__descriptor) \
    , 0, (char *)protobuf_c_empty_string, 0,NULL }


struct  _ChunkList
{
  ProtobufCMessage base;
  protobuf_c_boolean success;
  size_t n_chunks;
  Chunk **chunks;
};
#define CHUNK_LIST__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&chunk_list__descriptor) \
    , 0, 0,NULL }


struct  _ChunkRequest
{
  ProtobufCMessage base;
  char *path;
  int32_t chunk_id;
};
#define CHUNK_REQUEST__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&chunk_request__descriptor) \
    , (char *)protobuf_c_empty_string, 0 }


struct  _WriteChunk
{
  ProtobufCMessage base;
  char *path;
  int32_t chunk_id;
  size_t n_replicas;
  Replica **replicas;
};
#define WRITE_CHUNK__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&write_chunk__descriptor) \
    , (char *)protobuf_c_empty_string, 0, 0,NULL }


struct  _FileResponse
{
  ProtobufCMessage base;
  char *filename;
  ProtobufCBinaryData filedata;
};
#define FILE_RESPONSE__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&file_response__descriptor) \
    , (char *)protobuf_c_empty_string, {0,NULL} }


/*
 * CURRENTLY NOT USED
 */
struct  _CommitChunk
{
  ProtobufCMessage base;
  protobuf_c_boolean success;
  int32_t chunk_id;
  size_t n_replicas_success;
  Replica **replicas_success;
  size_t n_replicas_fail;
  Replica **replicas_fail;
};
#define COMMIT_CHUNK__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&commit_chunk__descriptor) \
    , 0, 0, 0,NULL, 0,NULL }


struct  _NewReplica
{
  ProtobufCMessage base;
  char *ip;
  int32_t port;
};
#define NEW_REPLICA__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&new_replica__descriptor) \
    , (char *)protobuf_c_empty_string, 0 }


struct  _ChunkCommitReport
{
  ProtobufCMessage base;
  char *ip;
  int32_t port;
  protobuf_c_boolean is_success;
};
#define CHUNK_COMMIT_REPORT__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&chunk_commit_report__descriptor) \
    , (char *)protobuf_c_empty_string, 0, 0 }


/* FileRequestRead methods */
void   file_request_read__init
                     (FileRequestRead         *message);
size_t file_request_read__get_packed_size
                     (const FileRequestRead   *message);
size_t file_request_read__pack
                     (const FileRequestRead   *message,
                      uint8_t             *out);
size_t file_request_read__pack_to_buffer
                     (const FileRequestRead   *message,
                      ProtobufCBuffer     *buffer);
FileRequestRead *
       file_request_read__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   file_request_read__free_unpacked
                     (FileRequestRead *message,
                      ProtobufCAllocator *allocator);
/* FileRequestWrite methods */
void   file_request_write__init
                     (FileRequestWrite         *message);
size_t file_request_write__get_packed_size
                     (const FileRequestWrite   *message);
size_t file_request_write__pack
                     (const FileRequestWrite   *message,
                      uint8_t             *out);
size_t file_request_write__pack_to_buffer
                     (const FileRequestWrite   *message,
                      ProtobufCBuffer     *buffer);
FileRequestWrite *
       file_request_write__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   file_request_write__free_unpacked
                     (FileRequestWrite *message,
                      ProtobufCAllocator *allocator);
/* Replica methods */
void   replica__init
                     (Replica         *message);
size_t replica__get_packed_size
                     (const Replica   *message);
size_t replica__pack
                     (const Replica   *message,
                      uint8_t             *out);
size_t replica__pack_to_buffer
                     (const Replica   *message,
                      ProtobufCBuffer     *buffer);
Replica *
       replica__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   replica__free_unpacked
                     (Replica *message,
                      ProtobufCAllocator *allocator);
/* Chunk methods */
void   chunk__init
                     (Chunk         *message);
size_t chunk__get_packed_size
                     (const Chunk   *message);
size_t chunk__pack
                     (const Chunk   *message,
                      uint8_t             *out);
size_t chunk__pack_to_buffer
                     (const Chunk   *message,
                      ProtobufCBuffer     *buffer);
Chunk *
       chunk__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   chunk__free_unpacked
                     (Chunk *message,
                      ProtobufCAllocator *allocator);
/* ChunkList methods */
void   chunk_list__init
                     (ChunkList         *message);
size_t chunk_list__get_packed_size
                     (const ChunkList   *message);
size_t chunk_list__pack
                     (const ChunkList   *message,
                      uint8_t             *out);
size_t chunk_list__pack_to_buffer
                     (const ChunkList   *message,
                      ProtobufCBuffer     *buffer);
ChunkList *
       chunk_list__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   chunk_list__free_unpacked
                     (ChunkList *message,
                      ProtobufCAllocator *allocator);
/* ChunkRequest methods */
void   chunk_request__init
                     (ChunkRequest         *message);
size_t chunk_request__get_packed_size
                     (const ChunkRequest   *message);
size_t chunk_request__pack
                     (const ChunkRequest   *message,
                      uint8_t             *out);
size_t chunk_request__pack_to_buffer
                     (const ChunkRequest   *message,
                      ProtobufCBuffer     *buffer);
ChunkRequest *
       chunk_request__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   chunk_request__free_unpacked
                     (ChunkRequest *message,
                      ProtobufCAllocator *allocator);
/* WriteChunk methods */
void   write_chunk__init
                     (WriteChunk         *message);
size_t write_chunk__get_packed_size
                     (const WriteChunk   *message);
size_t write_chunk__pack
                     (const WriteChunk   *message,
                      uint8_t             *out);
size_t write_chunk__pack_to_buffer
                     (const WriteChunk   *message,
                      ProtobufCBuffer     *buffer);
WriteChunk *
       write_chunk__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   write_chunk__free_unpacked
                     (WriteChunk *message,
                      ProtobufCAllocator *allocator);
/* FileResponse methods */
void   file_response__init
                     (FileResponse         *message);
size_t file_response__get_packed_size
                     (const FileResponse   *message);
size_t file_response__pack
                     (const FileResponse   *message,
                      uint8_t             *out);
size_t file_response__pack_to_buffer
                     (const FileResponse   *message,
                      ProtobufCBuffer     *buffer);
FileResponse *
       file_response__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   file_response__free_unpacked
                     (FileResponse *message,
                      ProtobufCAllocator *allocator);
/* CommitChunk methods */
void   commit_chunk__init
                     (CommitChunk         *message);
size_t commit_chunk__get_packed_size
                     (const CommitChunk   *message);
size_t commit_chunk__pack
                     (const CommitChunk   *message,
                      uint8_t             *out);
size_t commit_chunk__pack_to_buffer
                     (const CommitChunk   *message,
                      ProtobufCBuffer     *buffer);
CommitChunk *
       commit_chunk__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   commit_chunk__free_unpacked
                     (CommitChunk *message,
                      ProtobufCAllocator *allocator);
/* NewReplica methods */
void   new_replica__init
                     (NewReplica         *message);
size_t new_replica__get_packed_size
                     (const NewReplica   *message);
size_t new_replica__pack
                     (const NewReplica   *message,
                      uint8_t             *out);
size_t new_replica__pack_to_buffer
                     (const NewReplica   *message,
                      ProtobufCBuffer     *buffer);
NewReplica *
       new_replica__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   new_replica__free_unpacked
                     (NewReplica *message,
                      ProtobufCAllocator *allocator);
/* ChunkCommitReport methods */
void   chunk_commit_report__init
                     (ChunkCommitReport         *message);
size_t chunk_commit_report__get_packed_size
                     (const ChunkCommitReport   *message);
size_t chunk_commit_report__pack
                     (const ChunkCommitReport   *message,
                      uint8_t             *out);
size_t chunk_commit_report__pack_to_buffer
                     (const ChunkCommitReport   *message,
                      ProtobufCBuffer     *buffer);
ChunkCommitReport *
       chunk_commit_report__unpack
                     (ProtobufCAllocator  *allocator,
                      size_t               len,
                      const uint8_t       *data);
void   chunk_commit_report__free_unpacked
                     (ChunkCommitReport *message,
                      ProtobufCAllocator *allocator);
/* --- per-message closures --- */

typedef void (*FileRequestRead_Closure)
                 (const FileRequestRead *message,
                  void *closure_data);
typedef void (*FileRequestWrite_Closure)
                 (const FileRequestWrite *message,
                  void *closure_data);
typedef void (*Replica_Closure)
                 (const Replica *message,
                  void *closure_data);
typedef void (*Chunk_Closure)
                 (const Chunk *message,
                  void *closure_data);
typedef void (*ChunkList_Closure)
                 (const ChunkList *message,
                  void *closure_data);
typedef void (*ChunkRequest_Closure)
                 (const ChunkRequest *message,
                  void *closure_data);
typedef void (*WriteChunk_Closure)
                 (const WriteChunk *message,
                  void *closure_data);
typedef void (*FileResponse_Closure)
                 (const FileResponse *message,
                  void *closure_data);
typedef void (*CommitChunk_Closure)
                 (const CommitChunk *message,
                  void *closure_data);
typedef void (*NewReplica_Closure)
                 (const NewReplica *message,
                  void *closure_data);
typedef void (*ChunkCommitReport_Closure)
                 (const ChunkCommitReport *message,
                  void *closure_data);

/* --- services --- */


/* --- descriptors --- */

extern const ProtobufCMessageDescriptor file_request_read__descriptor;
extern const ProtobufCMessageDescriptor file_request_write__descriptor;
extern const ProtobufCMessageDescriptor replica__descriptor;
extern const ProtobufCMessageDescriptor chunk__descriptor;
extern const ProtobufCMessageDescriptor chunk_list__descriptor;
extern const ProtobufCMessageDescriptor chunk_request__descriptor;
extern const ProtobufCMessageDescriptor write_chunk__descriptor;
extern const ProtobufCMessageDescriptor file_response__descriptor;
extern const ProtobufCMessageDescriptor commit_chunk__descriptor;
extern const ProtobufCMessageDescriptor new_replica__descriptor;
extern const ProtobufCMessageDescriptor chunk_commit_report__descriptor;

PROTOBUF_C__END_DECLS


#endif  /* PROTOBUF_C_dfs_2eproto__INCLUDED */
